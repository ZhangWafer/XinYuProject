...
public static class PageUtility
{
    ...
    public static void PostPage(string relatedUrl, Dictionary<string, string> urlParameterDictionary)
    {
        var fullUrl = new StringBuilder(ConfigurationManager.AppSettings["WebClientUrlHeader"] + relatedUrl);

        int i = 0;
        foreach (var pair in urlParameterDictionary)
        {
            fullUrl.AppendFormat("{0}{1}={2}", ((i == 0) ? "?" : "&"), pair.Key, pair.Value);
            i += 1;
        }

        PostPage(fullUrl.ToString());
    }

    public static void PostPage(string fullUrl)
    {
        var client = new WebClient();
        client.OpenRead(fullUrl);
    }
